...
def ensure_required_users():
    """Ensure required users exist in the database"""
    try:
    ...
    for user_data in Required_users:
        ...
        if not existing_user:
            ...
            db.session.add(user)
            logger.info(f"Created required user: {user_data['username']}")
    
    db.session.commit()
    logger.info("Required users check completed")
