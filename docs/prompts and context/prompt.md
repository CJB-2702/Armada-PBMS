...
7. Logging System
   ...
   - Connection-aware database logging

   ```python
   # Basic usage
   from app.utils.logger import get_logger
   logger = get_logger()
   logger.info('Message', extra={'log_data': {'key': 'value'}})

   # Database logging
   from app.utils.logger import init_logger
   init_logger(app)  # Creates log table and connects handlers
   ```
   ...
   3. Database Logging:
      ...
      - Fallback to file/console if database unavailable
      - Structured data storage

   4. File Logging:
      - Rotating file handler (1KB max size)
      - JSON formatted output
      - 5 backup files maintained
      - Separate log file per logger instance
...
- The application startup is managed in `run.py`.
- On startup, the Flask app is created and an application context is established using `with app.app_context():`.
- The database tables are created (if they do not exist) using `db.create_all()`. This ensures all models, including logging tables, are present before any data or logging operations.
- The `initialize_system(debug=True)` function is called. This function:
    - Checks for the existence of essential and debug data (users, asset types, event types, locations, etc.).
    - Loads essential data if missing (system/admin users, core types, etc.).
    - Loads additional debug/test data if in debug mode.
    - All initialization steps are logged.
- The Flask app is then started in debug mode.
