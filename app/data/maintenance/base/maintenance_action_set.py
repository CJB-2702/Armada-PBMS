...
class MaintenanceActionSet(EventDetailVirtual, VirtualActionSet):
    ...
    def completion_percentage(self):
        """Calculate completion percentage based on actions (completed or skipped count as complete)"""
        if not self.actions:
            return 0
        completed_or_skipped_actions = len([a for a in self.actions if a.status in ['Completed', 'Skipped']])
        return (completed_or_skipped_actions / len(self.actions)) * 100
    ...
    def add_comment_to_event(self, user_id, comment_content, is_human_made=False):
        """Add a comment to the event"""
        if not self.event_id:
            raise ValueError("MaintenanceActionSet does not have an associated event")
    ...
    def start_maintenance(self, user_id, notes=None):
        ...
        is_human_made = bool(notes)  # Mark as human-made if notes are provided
        if notes:
            comment_content += f"\n\nNotes: {notes}"
        self.add_comment_to_event(user_id, comment_content, is_human_made=is_human_made)
    ...
    def complete_maintenance(self, user_id, notes=None):
        ...
        is_human_made = bool(notes)  # Mark as human-made if completion notes are provided
        if notes:
    ...
    def delay_maintenance(self, user_id, reason=None):
        """Delay the maintenance event"""
        self._set_status('Delayed', user_id)
        self.delay_notes = reason if reason else "Maintenance delayed"
        self.updated_by_id = user_id
        
        comment_content = f"Maintenance delayed: {self.task_name}"
        if reason:
            comment_content += f"\n\nDelay Reason: {reason}"
        
        self.add_comment_to_event(user_id, comment_content)
        self.update_event_status(user_id, "Delayed", comment_content)
    ...
    def all_actions_finished(self):
        """Check if all actions are either completed or skipped"""
        if not self.actions:
            return False
