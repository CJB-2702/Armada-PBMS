...
def build_models():
    ...
    # Import detail factories
    import app.models.assets.factories.detail_factory
    import app.models.assets.factories.asset_detail_factory
    import app.models.assets.factories.model_detail_factory
...
def get_detail_table_class(table_type):
    ...
    """
    from app.models.assets.factories.detail_factory import DetailFactory
...
def phase_2_init_data(build_data):
    ...
    for table_type, detail_data in sample_data.items():
        ...
        # Check if the table type exists in the factory registry
        from app.models.assets.factories.detail_factory import DetailFactory
        try:
            DetailFactory.get_detail_table_class(table_type_snake)
            table_exists = True
        except ValueError:
            table_exists = False
    ...
    # Insert model detail data
    _insert_model_details(build_data, system_user_id)
...
def _update_autogenerated_detail_table_data(table_type, asset, sample_data, system_user_id):
    ...
    from app.models.assets.factories.detail_factory import DetailFactory
    try:
        DetailFactory.get_detail_table_class(table_type)
    except ValueError:
        return
...
def _insert_models_for_automatic_detail_insertion(build_data, system_user_id):
    ...
    Uses MakeModelFactory for proper creation with detail table initialization
    ...
    for model_data in models_data:
        try:
            ...
            # Use MakeModelFactory to create the model (handles duplicate checking and detail table creation)
            make_model, created = MakeModelFactory.create_make_model_from_dict(
            ...
            if created:
                logger.info(f"Phase 2: Created model '{make_model.make} {make_model.model}' for automatic detail insertion")
            else:
                logger.info(f"Phase 2: Model '{make_model.make} {make_model.model}' already exists, skipping")
...
def _insert_model_details(build_data, system_user_id):
    """
    Insert model detail data from Model_Details section
    
    Args:
        build_data (dict): Build data from JSON file
        system_user_id (int): System user ID for audit fields
    """
    logger.info("Phase 2: Inserting model detail data")
    ...
    # Get model details data from Asset_Details section
    model_details_data = build_data.get('Asset_Details', {}).get('Model_Details', {})
    
    if not model_details_data:
        logger.info("Phase 2: No model details found")
        return
    
    for detail_key, detail_data in model_details_data.items():
        try:
            # Get make and model from detail data
            ...
            if not make or not model or not detail_type:
                logger.warning(f"Phase 2: Skipping {detail_key} - missing make, model, or detail_type")
                continue
            
            # Find the make/model
            make_model = MakeModel.query.filter_by(make=make, model=model).first()
            if not make_model:
                logger.warning(f"Phase 2: Make/Model not found for {make} {model}")
                continue
            
            # Remove meta fields from detail_data
            detail_record_data = {k: v for k, v in detail_data.items() if k not in ['make', 'model', 'detail_type']}
            
            # Convert date strings to date objects
            detail_record_data = convert_date_strings(detail_record_data)
            ...
            if detail_type == 'emissions_info':
                ...
                if not existing:
                    ...
                    logger.info(f"Phase 2: Created emissions_info for {make} {model}")
                else:
                    logger.info(f"Phase 2: emissions_info already exists for {make} {model}")
                ...
                if not existing:
                    ...
                    db.session.add(detail_record)
                    logger.info(f"Phase 2: Created model_info for {make} {model}")
                else:
                    logger.info(f"Phase 2: model_info already exists for {make} {model}")
            else:
                logger.warning(f"Phase 2: Unknown detail_type {detail_type} for {detail_key}")
                
        except Exception as e:
            logger.error(f"Phase 2: Error creating model detail {detail_key}: {e}")
            continue
    
    # Commit all changes
    try:
        db.session.commit()
        logger.info("Phase 2: Successfully committed all model detail data")
    except Exception as e:
        db.session.rollback()
        logger.error(f"Phase 2: Error committing model detail data: {e}")
        raise
...
def _insert_assets_for_automatic_detail_insertion(build_data, system_user_id):
    ...
    Uses AssetFactory for proper creation with detail table initialization
    ...
    for asset_data in assets_data:
        try:
            ...
            # Use AssetFactory to create the asset (handles duplicate checking and detail table creation)
            ...
            if created:
                logger.info(f"Phase 2: Created asset '{asset.name}' for automatic detail insertion")
            else:
                logger.info(f"Phase 2: Asset '{asset.name}' already exists, skipping")
