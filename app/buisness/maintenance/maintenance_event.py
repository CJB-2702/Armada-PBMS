...
class MaintenanceEvent:
    ...
    def skipped_actions(self) -> int:
        """Get number of skipped actions"""
        return len([a for a in self.actions if a.status == 'Skipped'])
    
    @property
    def completed_or_skipped_actions(self) -> int:
        """Get number of completed or skipped actions"""
        return len([a for a in self.actions if a.status in ['Completed', 'Skipped']])
    
    @property
    def all_actions_finished(self) -> bool:
        """Check if all actions are either completed or skipped"""
        if self.total_actions == 0:
            return False
        return self.completed_or_skipped_actions == self.total_actions
    ...
    def completion_percentage(self) -> float:
        ...
        if self.total_actions == 0:
            return 0.0
        completed_or_skipped = len([a for a in self.actions if a.status in ['Completed', 'Skipped']])
        return (completed_or_skipped / self.total_actions) * 100
