...
class AssetTypeDispatchDetailTableSet(UserCreatedBase, db.Model):
    ...
    def create_dispatch_detail_table_rows(cls, dispatch_id, asset_id):
        ...
        print(f"DEBUG: AssetTypeDispatchDetailTableSet.create_dispatch_detail_table_rows called for dispatch {dispatch_id}")
        try:
            ...
            print(f"DEBUG: Found {len(detail_configs)} dispatch detail configurations")
            
            for config in detail_configs:
                print(f"DEBUG: Creating dispatch detail row for {config.dispatch_detail_table_type}")
                cls._create_single_dispatch_detail_row(config, dispatch_id)
                
        except Exception as e:
            print(f"Error creating asset type dispatch detail table rows for dispatch {dispatch_id}: {e}")
    ...
    def _create_single_dispatch_detail_row(cls, config, dispatch_id):
        ...
        print(f"DEBUG: _create_single_dispatch_detail_row called for {config.dispatch_detail_table_type}")
        try:
            ...
            print(f"DEBUG: Dispatch detail table class path: {dispatch_detail_table_class_path}")
            ...
            print(f"DEBUG: Dispatch detail table class: {dispatch_detail_table_class}")
            
            # Create the dispatch detail table row - check for duplicates first
            print(f"DEBUG: Creating dispatch detail row for dispatch_id={dispatch_id}")
            ...
            print(f"DEBUG: Created dispatch detail row: {dispatch_detail_row}")
            
            # Add to session (don't commit - let the main transaction handle it)
            db.session.add(dispatch_detail_row)
            print(f"DEBUG: Added dispatch detail row to session")
