...
from app import db
from app.models.inventory.base import (
    PackageHeader,
    PartArrival,
    PurchaseOrderLine
)
...
class PartArrivalManager:
    ...
    def receive_parts(package_id, po_line_id, quantity, condition, user_id, 
        ...
        if po_line.purchase_order.event:
            comment = Comment(
                event_id=po_line.purchase_order.event_id,
                comment_text=f"Received {quantity} of {po_line.part.part_name} in package {package.package_number}",
                created_by_id=user_id
            )
    ...
    def inspect_arrival(arrival_id, accepted_qty, rejected_qty, notes, user_id):
        ...
        if po.event:
            comment = Comment(
                ...
                comment_text=f"Inspection complete: {accepted_qty} accepted, {rejected_qty} rejected",
                created_by_id=user_id
            )
            db.session.add(comment)
    ...
    def accept_arrival(arrival_id, user_id):
        ...
        if po.event:
            comment = Comment(
                event_id=po.event_id,
                comment_text=f"Accepted {arrival.quantity_accepted} units into inventory",
                created_by_id=user_id
            )
            db.session.add(comment)
    ...
    def reject_arrival(arrival_id, reason, user_id):
        ...
        if po.event:
            comment = Comment(
                event_id=po.event_id,
                comment_text=f"Rejected {arrival.quantity_received} units: {reason}",
                created_by_id=user_id
            )
            db.session.add(comment)
