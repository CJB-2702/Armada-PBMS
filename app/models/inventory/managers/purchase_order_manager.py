...
class PurchaseOrderManager:
    ...
    def submit_order(po_id, user_id):
        ...
        if po.event:
            ...
            comment = Comment(
                ...
                comment_text=f"Purchase order submitted for ordering",
                created_by_id=user_id
            )
    ...
    def cancel_order(po_id, reason, user_id):
        ...
        if po.event:
            ...
            comment = Comment(
                event_id=po.event_id,
                comment_text=f"Purchase order cancelled: {reason}",
                created_by_id=user_id
            )
            db.session.add(comment)
    ...
    def check_completion_status(po_id, user_id):
        ...
        if old_status != po.status:
            ...
            if po.event:
                ...
                comment = Comment(
                    event_id=po.event_id,
                    comment_text=f"Purchase order status changed to {po.status}",
                    created_by_id=user_id
                )
                db.session.add(comment)
